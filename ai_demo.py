...
Original file is located at
    https://colab.research.google.com/drive/1i7z5_AuY-VmXCoFy2_YYn8wGe53aJ56b

# DECISION TREE AND NAIVE BAYES

Decision Tree and Naive Bayes are two popular classification algorithms. Both are widely used in various applications such as *spam filtering, fraud detection, and medical diagnosis*.

A Decision Tree is a supervised learning algorithm that is widely used for classification and regression tasks. It works by splitting the dataset into subsets based on the feature that provides the highest information gain. The end result is a tree-like structure where each internal node represents a decision based on a feature, and the leaf nodes represent the final classification or output.


##How Decision Trees Work
###1. Splitting the Dataset:
The algorithm selects a feature that best splits the dataset into distinct classes.
###2. Tree Structure:
The decision tree is built recursively. Each node represents a feature and the branches represent decisions based on that feature.
###3. Termination:
The tree continues splitting the dataset until it cannot be split further, either because the data is perfectly classified or the maximum depth of the tree is reached.
...
1. Theoretical Differences
    ####     Model Type:
    Decision Tree: A non-parametric model that doesn’t assume any particular distribution of the data.

    Naive Bayes: A parametric model that assumes conditional independence between features.

    Basis:
    Decision Tree: Works on the principle of recursively splitting the data based on feature importance.
    
    Naive Bayes: Based on probability theory and Bayes’ theorem, using prior knowledge of the likelihood of features given the class.

2. Handling of Data
    ####  Feature Interactions:
    Decision Tree: Capable of capturing complex feature interactions as it recursively splits the data.

    Naive Bayes: Assumes independence between features, which may result in poor performance if there are strong feature dependencies.


####Categorical and Continuous Features:
Decision Tree: Handles both categorical and continuous features naturally without requiring much preprocessing.

Naive Bayes: Can handle both, but continuous features require assumptions about their distribution, typically Gaussian.
"""
...
This project predicts a student’s performance level (low, medium, or high) based on demographic, social, and academic features.

#### Instead of predicting exact marks, we classify students into categories to understand learning patterns more easily.

We use the Student Performance dataset, which includes details like age, study time, absences, previous grades, and family/school conditions. The final grade (G3) is converted into performance levels:
...
Decision Tree	Learns rules from data and splits it into groups	Easy to understand and visualize.

*Gaussian Naive Bayes*

Uses probability to classify students based on feature values	Very fast and works well with simple assumptions.

###What the code does

1. Loads and prepares the dataset

2. Converts the final grade into performance classes

3. Preprocesses numeric and categorical features

4. Trains both models on student data

5. Evaluates accuracy and shows confusion matrices

6. Visualizes the decision tree for interpretation

7. Predicts performance for a new student profile

### Key Insight

Machine learning can help identify which factors (like previous grades, study time, and absences) influence student performance and can assist educators in providing targeted support.
"""